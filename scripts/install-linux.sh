#!/usr/bin/env sh
set -eu
PROJECT="Exposure DNA"; CMD="exposuredna"; INSTALL_ROOT="${HOME}/.exposuredna"; VENV="$INSTALL_ROOT/venv"; BIN_DIR="${HOME}/.local/bin"
...
if [ -n "${SRIC_CORE_SOURCE:-}" ]; then
  ...
  "$VENV/bin/python" -m pip install --upgrade "$SRIC_CORE_SOURCE"
fi
"$VENV/bin/python" -m pip install --upgrade "$REPO_ROOT" || { echo "Installation failed. Exposure DNA requires sric-core>=0.5,<0.6, resolved automatically unless SRIC_CORE_SOURCE is set." >&2; exit 3; }
