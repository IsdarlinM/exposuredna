...
rem Resolve Exposure DNA and the explicit SRIC source atomically; sric-core is
...
if defined SRIC_CORE_SOURCE (
  ...
  "%VENV%\Scripts\python.exe" -m pip install --upgrade --force-reinstall -c "%CONSTRAINTS%" "%SRIC_CORE_SOURCE%" "%REPO_ROOT%" || (echo Atomic Exposure DNA/SRIC installation failed.& exit /b 3)
) else (
  if not exist "%FIRST_PARTY%" (echo Missing first-party dependency manifest: %FIRST_PARTY%& exit /b 3)
  "%VENV%\Scripts\python.exe" -m pip install --upgrade --force-reinstall -c "%CONSTRAINTS%" -r "%FIRST_PARTY%" "%REPO_ROOT%" || (echo Atomic Exposure DNA/SRIC installation failed.& exit /b 3)
)
