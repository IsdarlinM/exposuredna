...
- Adopted professional offline Segoe UI Variable/Aptos/system typography and Cascadia Code/SFMono/Consolas evidence output with a restrained graphite/slate + teal palette; no external font or CDN dependency is required.
- Preserved exact CLI/Web parity, fixed-runner `shell=False`, no user-supplied argv, Scope/Policy/Rate/Approval controls, CSRF protection, redaction and evidence/temporal/human-review ownership semantics.
...
- Added Security Workspace v3 E2E coverage plus an exhaustive offline dispatch/validation smoke for every public Exposure DNA CLI command.
- Hosted jobs that never execute steps are not treated as PASS; merge/release state and execution evidence remain separate.

## 0.5.12 - 2026-08-10
- Aligned Exposure DNA package metadata, runtime lock, bootstrap, `doctor` and Linux/Windows installers to SRIC Core `>=0.5.12,<0.6`.
...
- Updated installer/runtime regressions for the exact pin/lock/transition chain while retaining exhaustive CLI help/argument-to-Web parity and conservative evidence/temporal/human-review ownership semantics.
