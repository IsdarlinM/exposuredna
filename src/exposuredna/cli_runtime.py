...
@app.command()
def web(workspace:str,host:str="127.0.0.1",port:int=8769,root:Path=typer.Option(rd(),"--root"))->None:
    if host not in {"127.0.0.1","localhost","::1"}: typer.echo("Non-loopback binding disabled until authenticated TLS mode is configured.",err=True); raise typer.Exit(4)
    import uvicorn; uvicorn.run(create_app(wp(workspace,root)),host=host,port=port)
...
def evidence_add(workspace:str,file:Path,source:str=typer.Option("user","--source"),media_type:str=typer.Option("application/octet-stream","--media-type"),redacted:bool=typer.Option(False,"--redacted"),root:Path=typer.Option(rd(),"--root"))->None:
    ...
    workspace_path=wp(workspace,root); Workspace.open(workspace_path); ref=EvidenceStore(workspace_path/"evidence").put_bytes(file.read_bytes(),media_type=media_type,provenance=Provenance(provenance_type=ProvenanceType.USER_INPUT,source=source,method="cli_evidence_add",tool_version=__version__),redacted=redacted); typer.echo(json.dumps(ref.model_dump(mode="json"),indent=2))

@app.command("ai")
def ai_status()->None: typer.echo(json.dumps({"mode":"disabled","provider":"disabled","cloud_uploads":False},indent=2))
@app.command("plugins")
...
def update(check:bool=typer.Option(False,"--check"),manifest:Optional[str]=typer.Option(None,"--manifest"),public_key:Optional[Path]=typer.Option(None,"--public-key"))->None:
    import os
    source=manifest or os.getenv("EXPOSUREDNA_RELEASE_MANIFEST_URL");key=public_key or(Path(os.environ["EXPOSUREDNA_RELEASE_PUBLIC_KEY"]) if os.getenv("EXPOSUREDNA_RELEASE_PUBLIC_KEY") else None)
    ...
    try:status=perform_update(manifest_source=source,public_key_path=key,expected_product="exposuredna",current_version=__version__,check_only=check)
