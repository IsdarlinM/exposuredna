...
def update(
    ...
    """Check/install a signed Exposure DNA wheel, including explicit same-version reinstalls."""

    if check and force:
        typer.echo("--check and --force cannot be used together.", err=True)
        raise typer.Exit(2)
    source = manifest or os.getenv("EXPOSUREDNA_RELEASE_MANIFEST_URL")
    key = public_key or (
        Path(os.environ["EXPOSUREDNA_RELEASE_PUBLIC_KEY"])
        if os.getenv("EXPOSUREDNA_RELEASE_PUBLIC_KEY")
        else None
    )
    if not source or key is None:
        typer.echo(
            "No trusted release channel configured. Provide --manifest and --public-key, "
            "or EXPOSUREDNA_RELEASE_MANIFEST_URL/EXPOSUREDNA_RELEASE_PUBLIC_KEY.",
            err=True,
        )
        raise typer.Exit(2)
    try:
