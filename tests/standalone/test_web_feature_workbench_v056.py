...
from exposuredna.api_all import create_app
...
def test_every_public_cli_command_and_argument_is_represented_in_workbench() -> None:
    cli = {item["path"]: item for item in build_command_catalog("exposuredna.cli_all")}
    web = {item["path"]: item for item in build_feature_catalog("exposuredna.cli_all")}
...
def test_every_public_cli_command_help_exposes_options_and_required_arguments() -> None:
    catalog = build_command_catalog("exposuredna.cli_all")
...
def test_workbench_dashboard_and_native_features(tmp_path) -> None:
    ...
    assert "All Features" not in dashboard.text
    ...
    assert {item["path"] for item in payload["features"]} == {
        item["path"] for item in build_command_catalog("exposuredna.cli_all")
    }

    for route in ("/api/graph", "/api/dna", "/api/resolution-queue", "/api/lineage", "/api/external-correlations", "/api/jobs", "/api/notebook"):
