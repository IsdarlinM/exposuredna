# Test Evidence — Exposure DNA v0.5.12 Candidate

## Candidate review — 2026-08-10

Exposure DNA 0.5.12 aligns package metadata, runtime lock, bootstrap, `doctor`, Linux/Termux installer and Windows installer to SRIC Core `>=0.5.12,<0.6` and pins signed SRIC main commit `4dd0ad417e55fc76fb67d582ec50234bffff2876`.

Candidate regressions verify the exact first-party pin/lock, required `sric.web_console`, `sric.web_workbench`, `sric.web_catalog` and `sric.web_runtime` modules, the fixed signed 0.5.5→0.5.12 sequential repair chain, same-version 0.5.12 repair, degraded Workbench 503 behavior, every public CLI help form and exact ordered CLI/Web parameter parity. Existing suites cover organization graph/DNA dimensions, entity resolution, counter-evidence, human review, temporal relationships, lineage, passive adapters, native API/Web resources and conservative ownership semantics.
...
The four checks covered redacted catalog HTTP 503 behavior, terminal-job/SSE retention, final-wait background reaping and Job Engine persistence redaction. This is shared-runtime evidence only, not a full Exposure DNA repository/platform/browser PASS.

## Exact-commit gate status

**THE COMPLETE v0.5.12 EXPOSURE DNA TEST/RELEASE GATES HAVE NOT EXECUTED.**
