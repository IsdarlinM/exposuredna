...
import exposuredna.cli_update as cli_update
...
def test_update_force_uses_official_channel_without_manifest_or_key(monkeypatch) -> None:
    ...
    monkeypatch.delenv("EXPOSUREDNA_RELEASE_MANIFEST_URL", raising=False)
    monkeypatch.delenv("EXPOSUREDNA_RELEASE_PUBLIC_KEY", raising=False)
    monkeypatch.setattr(cli_update, "perform_product_update", fake_update)

    result = CliRunner().invoke(app, ["update", "--force"])
    assert result.exit_code == 0, result.output
    assert seen["expected_product"] == "exposuredna"
