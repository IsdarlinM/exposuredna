...
import exposuredna.sric_bootstrap as bootstrap
from exposuredna.api_all import _mount_degraded_workbench
from exposuredna.cli_all import app, normalize_help_argv
...
def test_every_exposure_command_and_param_is_web_represented_and_all_help_forms() -> None:
    cli = build_command_catalog("exposuredna.cli_all")
    web = build_feature_catalog("exposuredna.cli_all")
    assert feature_contract("exposuredna.cli_all")["complete"] is True
    ...
    for path, command in cli_by_path.items():
        ...
        normalized = normalize_help_argv(["exposuredna", *args, "help"])
